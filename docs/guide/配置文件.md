...
:::details 自定义签到, 支持图像 LLM 理解图片验证码并点击按钮 (**`templ_ai`**)

可以使用 `templ_ai<xxx>` 格式启用, `xxx` 为机器人的用户名.
...
[site]
checkiner = ["templ_ai<moumou_bot>"]

[checkiner.'templ_ai<moumou_bot>']
name = "某某站点"
templ_panel_keywords = "请选择功能"
bot_checkin_button = "签到"
bot_checkin_caption_pat = "请选择正确答案"
```

该模板复用 `templ_a` 的面板点击逻辑, 但当机器人发来图片且满足 `bot_use_captcha = true` 时, 会把该消息识别为验证码图片, 调用 OpenAI 兼容视觉模型返回答案, 再从该图片消息自带的按钮中匹配并点击.

需要在配置文件中提供 OpenAI 兼容接口配置:
...
| ----- | ----- | ---- | ------ |
| `name` | `str` | 签到器显示名称 | |
| `templ_panel_keywords` | `str`/`list` | 机器人主菜单文字包含的关键词 | |
| `bot_checkin_button` | `str`/`list` | 机器人主菜单需点击的签到按钮 | `["签到", "簽到"]` |
| `use_button_answer` | `bool` | 点击按钮后等待并识别响应 | `true` |
| `bot_text_ignore_answer` | `str`/`list` | 忽略的响应文本 | `["Done"]` |
| `bot_fail_keywords` | `str`/`list` | 签到错误将重试时检测的关键词 | (使用内置) |
| `bot_success_keywords` | `str`/`list` | 成功时检测的关键词 | (使用内置) |
| `bot_success_pat` | `str` | 当接收到成功消息后, 从消息中提取数字的 Regex 模式, 只能含有 1~2 个捕获区 | `"(\d+)[^\d]*(\d+)"` |
| `bot_captcha_len` | `int` | 验证码长度的可能范围 | `[]` |
| `bot_text_ignore` | `str`/`list` | 当含有列表中的关键词, 即忽略该消息 | `[]` |
| `bot_checkin_caption_pat` | `str` | 当 Bot 返回图片时, 仅当符合该 Regex 才识别为验证码 | (不限制) |
| `bot_checkin_cmd` | `str` | 发送的签到命令 | `"/start"` |
| `bot_use_captcha` | `bool` | 当 Bot 返回图片时, 识别为验证码并触发图像模型 | `true` |
| `provider` | `str` | 自定义 AI 提供商名称, 仅用于日志展示 | `"openai-compatible"` |
| `base_url` | `str` | OpenAI 兼容接口 Base URL | |
| `api_key` | `str` | 站点独立覆盖的 OpenAI 兼容接口 API Key, 默认读 `[checkiner.ai].api_key` | |
| `model` | `str` | 站点独立覆盖的模型名, 默认读 `[checkiner.ai].model` | `"gpt-4.1-mini"` |
| `model_id` | `str` | 兼容旧配置, 等同于 `model` | |
| `llm_prompt` | `str` | 发送给图像模型的自定义提示词 | (内置提示词) |
| `llm_timeout` | `int` | 图像模型请求超时时间 (秒) | `60` |
| `llm_button_match_threshold` | `int` | 模型答案与按钮文本的最低匹配分数 | `70` |
...
| `bot_token` | `str` | 自定义 Telegram Bot Token, 配置后将直接使用你自己的机器人发送通知 | |
| `chat_id` | `str`/`int` | 自定义 Telegram 目标 chat_id, 需与 `bot_token` 一起配置 | |
...
当 `bot_token` 和 `chat_id` 已配置时, 将优先使用你自己的 Telegram 机器人发送, 不再依赖 `@embykeeper_bot`.

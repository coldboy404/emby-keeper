...
该模板复用 `templ_a` 的面板点击逻辑, 但当机器人发来图片且满足 `bot_use_captcha = true` 时, 会把该消息识别为验证码图片, 调用图像理解模型返回答案, 再从该图片消息自带的按钮中匹配并点击.
...
| `api_key` | `str` | 站点独立覆盖的智谱 API Key, 默认读 `[checkiner.ai].api_key` | |
| `model_id` | `str` | 站点独立覆盖的智谱图像模型 ID, 默认读 `[checkiner.ai].model_id` | `"glm-4.1v-thinking-flashx"` |
